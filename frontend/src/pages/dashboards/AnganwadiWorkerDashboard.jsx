...
const AnganwadiWorkerDashboard = () => {
    ...
    const getPageSummary = () => {
        ...
        children.forEach(c => {
            const tier = (c.risk_tier || '').toLowerCase();
            if (tier.includes('high')) riskCounts.high++;
            else if (tier.includes('moderate')) riskCounts.moderate++;
            else riskCounts.low++;
        });

        text += " " + t('parent.narration.risk_distribution')
            .replace('{high}', riskCounts.high)
            .replace('{moderate}', riskCounts.moderate)
            .replace('{low}', riskCounts.low);
    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="relative overflow-hidden bg-gradient-to-r from-green-600 to-teal-600 rounded-2xl p-6 text-white flex flex-wrap justify-between items-center gap-4">
                ...
                <div className="absolute -left-10 -bottom-10 w-40 h-40 bg-teal-400/20 rounded-full blur-3xl" />
            </div>
